...
locale InfoFlow = event ptran petran fin_com
  ...
  assumes info_vpeq_trans : "\<forall> a b c u. (a \<sim> u \<sim> b) \<and> (b \<sim> u \<sim> c) \<longrightarrow> (a \<sim> u \<sim> c)"
    ...
    and   info_step_def : "step a \<equiv> {(P, Q). (\<Gamma> \<turnstile> P -pes-(actk a)\<rightarrow> Q) \<and> 
    ...
    (*and   info_step_def : "step a \<equiv> {(P,Q). (P -pes-(actk a)\<rightarrow> Q) \<and> (*domc dome P (actk a)*) 
...
proof
  ...
  then show "local_respect"
    ...
    apply (drule_tac a = a and b = d and c = aa and d = ab and e = b in all5_imp2D)
      apply (simp add: reachable0_equiv, simp)

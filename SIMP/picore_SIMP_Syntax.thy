...
syntax
  ...
  "_Event"     :: "['a, 'a, 'a] \<Rightarrow> ('l,'k,'s,'s com option) event" ("(EVENT _ WHERE _ THEN _ END)" [0,0,0] 61)
  "_Event2"     :: "['a, 'a] \<Rightarrow> ('l,'k,'s,'s com option) event" ("(EVENT _ THEN _ END)" [0,0] 61)
  "_Event_a"     :: "['a, 'a, 'a] \<Rightarrow> ('l,'k,'s,'s com option) event" ("(EVENT\<^sub>A _ WHERE _ THEN _ END)" [0,0,0] 61)
...
translations
  ...
  "EVENT\<^sub>A l WHERE g THEN bd END" \<rightleftharpoons> "EVENT l THEN (AWAIT g THEN bd END) END"

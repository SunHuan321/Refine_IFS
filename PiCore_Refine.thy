...
assumes
  ...
  action_refine : "InfoFlow\<^sub>c.reachablec0 C\<^sub>c \<longrightarrow> (C\<^sub>c, C\<^sub>c') \<in> step\<^sub>c a\<^sub>c \<longrightarrow>
                  (\<forall>Cc Cc'. (Cc, Cc') \<in> step\<^sub>c a\<^sub>c \<longrightarrow> sim_s Cc = sim_s Cc') \<or>
                  (\<exists>a\<^sub>a. \<forall>Cc Cc'. (Cc, Cc') \<in> step\<^sub>c a\<^sub>c \<longrightarrow> ((sim_s Cc, sim_s Cc') \<in> step\<^sub>a a\<^sub>a)
                  \<and> domain a\<^sub>c = domain a\<^sub>a)" and
  intefere_same : "interference\<^sub>c  = interference\<^sub>a" and 
  sim_ifs : "gets (sim_s C\<^sub>c) \<sim>\<^sub>a d \<sim>\<^sub>a gets (sim_s C'\<^sub>c) = gets C\<^sub>c \<sim>\<^sub>c d \<sim>\<^sub>c gets C'\<^sub>c"   
begin

lemma "InfoFlow\<^sub>c.reachablec0 s\<^sub>c \<longrightarrow> (s\<^sub>c, t\<^sub>c) \<in> step\<^sub>c a\<^sub>c \<longrightarrow>
                  ...
                  \<and> domain a\<^sub>c = domain a\<^sub>a)"
  using action_refine by blast
interpretation SM_Refine C0\<^sub>c step\<^sub>c domain InfoFlow\<^sub>c.obsC InfoFlow\<^sub>c.vpeqC interference\<^sub>c
                         C0\<^sub>a step\<^sub>a domain InfoFlow\<^sub>a.obsC InfoFlow\<^sub>a.vpeqC interference\<^sub>a
                         sim_s
proof
  ...
  show "sim_s C0\<^sub>c = C0\<^sub>a"
    by (simp add: init_sim)
  show "\<And>s\<^sub>c t\<^sub>c a\<^sub>c. SM_IFS.reachable0 C0\<^sub>c step\<^sub>c s\<^sub>c \<longrightarrow> (s\<^sub>c, t\<^sub>c) \<in> step\<^sub>c a\<^sub>c \<longrightarrow> 
       (\<forall>sc sc'. (sc, sc') \<in> step\<^sub>c a\<^sub>c \<longrightarrow> sim_s sc = sim_s sc') \<or> (\<exists>a\<^sub>a. \<forall>sc sc'. ((sc, sc') \<in> step\<^sub>c a\<^sub>c 
       \<longrightarrow> (sim_s sc, sim_s sc') \<in> step\<^sub>a a\<^sub>a) \<and> domain a\<^sub>c = domain a\<^sub>a)"
    by (meson InfoFlow\<^sub>c.reachable0_equiv action_refine)
  show "interference\<^sub>c = interference\<^sub>a"
    using  intefere_same by force

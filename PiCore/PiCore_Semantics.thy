...
lemma ptran_not_none: "(\<Gamma> \<turnstile> (Q,s) -c\<rightarrow> (P,t)) \<Longrightarrow> Q \<noteq> fin_com"
  using none_no_tran apply(simp add:ptran'_def)  by metis
...
where
  AnonyEvent: "\<Gamma> \<turnstile> (P, s) -c\<rightarrow> (Q, t) \<Longrightarrow> \<Gamma> \<turnstile> (AnonyEvent P, s, x) -et-(Cmd P)\<sharp>k\<rightarrow> (AnonyEvent Q, t, x)"
